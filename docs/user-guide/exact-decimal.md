...
Exact decimal mode reduces the rounding artifacts of floating-point arithmetic for decimal-heavy calculations.
...
In standard floating-point mode this can display as `0.30000000000000004`. In exact decimal mode the result is presented cleanly:
...
- Decimal quantities are carried with higher precision through supported operations.
...
Exact decimal mode cannot make irrational values exact. Expressions such as `sqrt(2)` still produce an approximation.
