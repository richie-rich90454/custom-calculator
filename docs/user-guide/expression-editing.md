...
The expression editor is the heart of the calculator. This page explains the
editing model: how the cursor behaves, how insertion works, and how selection
and deletion behave.

## The cursor

The cursor (caret) always sits at the logically correct insertion point after
every action.
...
Pressing the `sin` button inserts the function call and moves the cursor to the
...
Template buttons insert a complete block with placeholders. The cursor lands on
the first placeholder, ready for your input.
...
Placeholders such as `x`, `a`, `b`, `n`, `0`, `5`, and `10` are starting
points that you replace with your own values.
...
When text is selected, pressing a function button wraps the selection in the
function call, with the cursor after the wrapped argument:

```
sin(x+1|)
```

Selection wrapping is the predictable behavior used for all single-argument
functions.
...
- If focus moves to a button, a synthetic caret indicator remains visible at
