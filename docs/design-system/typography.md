...
The design system uses two font families: a UI sans-serif and a display
monospace.
...
The expression editor uses the monospace display font. Monospace keeps every
character the same width, which makes cursor measurement and editing feel
stable and precise.
...
The expression editor uses a relaxed line height (`1.5`) so the caret and
selection remain clearly visible.
...
Fonts are loaded through `@fontsource/noto-sans` and
`@fontsource/noto-sans-mono` in the root package.
