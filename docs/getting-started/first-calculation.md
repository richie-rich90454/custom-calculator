...
This page walks through a complete calculation session: entering an
expression, editing it, evaluating it, and continuing with the result.
...
You can move the cursor with the arrow keys and edit anywhere in the
expression.
...
For example, with the caret at the end of `45 * 3`, pressing `Backspace`
removes the `3`, giving `45 *`.
...
The result `135` is shown in the result line, and the expression is stored in
history.
...
- Pressing an **operator** continues from the previous result, using it as the
  left operand.

This keeps the natural flow of a scientific calculator: you rarely need to
...
- [Interface tour](/getting-started/interface-tour) describes every region of
  the screen.
- [Expression editing](/user-guide/expression-editing) covers the editing model
  in depth.
