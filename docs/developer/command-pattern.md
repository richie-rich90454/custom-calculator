...
Every mutation in the calculator is modeled as a command object. Commands keep the controller small and make each operation independently testable.

## The command shape

Commands extend `AbstractCalculatorCommand` and implement a single `execute` method that transforms session state:
...
`CalculatorSessionState` is immutable. Commands return a new state via `copyWith`, never mutating the input.
