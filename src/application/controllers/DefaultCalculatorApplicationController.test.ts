...
describe("DefaultCalculatorApplicationController", () => {
    ...
    function createState(overrides: Partial<CalculatorSessionState> = {}): CalculatorSessionState {
    ...
    it("applies a text edit with its cursor position", () => {
        const nextState = controller.applyInsertion(createState({ resultText: "42" }), {
            text: "sin(",
            cursorPosition: 4,
            selectionStart: 4,
            selectionEnd: 4,
    ...
    it("evaluates an expression", () => {
        const nextState = controller.evaluateExpression(createState({ expressionText: "2+3" }));
    ...
    it("cycles the angle mode", () => {
        const nextState = controller.cycleAngleMode(createState({ angleMode: AngleMode.DEG }));

        expect(nextState.angleMode).not.toBe(AngleMode.DEG);
    });

    it("changes the numeric mode", () => {
        const nextState = controller.changeNumericMode(createState(), NumericMode.EXACT_DECIMAL);
    ...
    it("subtracts the current result from memory", () => {
        ...
        const nextState = controller.memorySubtract(withMemory.copyWith({ resultText: "3" }));
    ...
    it("clears memory", () => {
        const nextState = controller.memoryClear(createState({ memoryValueText: "42" }));
