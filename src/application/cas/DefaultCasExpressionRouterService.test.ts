...
function buildDescriptor(overrides: Partial<CasBlockDescriptor> = {}): CasBlockDescriptor {
    return {
...
describe("DefaultCasExpressionRouterService", () => {
    ...
    it("differentiates a derivative block with its variable", () => {
        ...
        expect(casService.differentiateExpression).toHaveBeenCalledWith("x^2+x", "x");
    });
    ...
    it("surfaces CAS engine errors as inline errors", () => {
        ...
        const resolution = router.routeBlock(buildDescriptor(), true, failingCasService);
