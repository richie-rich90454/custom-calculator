...
export class DefaultNumericIntegrationService
  implements NumericIntegrationService
{
  ...
  public integrate(
    ...
    try {
      const wholeInterval = this.computeSimpsonInterval(
        integrand,
        lowerBound,
        upperBound
      );
  ...
  private adaptivelyRefine(
    ...
    const leftInterval = this.computeSimpsonInterval(
      integrand,
      lowerBound,
      midpoint
    );
    const rightInterval = this.computeSimpsonInterval(
      integrand,
      midpoint,
      upperBound
    );
  ...
  private computeSimpsonInterval(
    ...
    return (
      ...
      (integrand(lowerBound) +
        4 * integrand(midpoint) +
        integrand(upperBound))
    );
  }

  private isNonFiniteEvaluationError(error: unknown): boolean {
    return (
      error instanceof Error && error.message.includes("non finite value")
    );
  }
}
