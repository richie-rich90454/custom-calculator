...
const numericDifferentiationService = new DefaultNumericDifferentiationService(evaluator);
const numericIntegrationService = new DefaultNumericIntegrationService(evaluator);
...
describe("DefaultCalculusExpressionRouterService", () => {
    it("routes a symbolic derivative block", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createUnary(CalculusOperationKind.DERIVATIVE, "x^2", "x"),
            AngleMode.RAD,
        );
    ...
    it("routes a numeric integral block", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createBounded(CalculusOperationKind.INTEGRAL, "x^2", "x", 0, 1),
            AngleMode.RAD,
        );
    ...
    it("routes a symbolic integrate block", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createUnary(CalculusOperationKind.INTEGRATE, "x", "x"),
            AngleMode.RAD,
        );
    ...
    it("routes a sum block", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createBounded(CalculusOperationKind.SUM, "n^2", "n", 1, 10),
            AngleMode.RAD,
        );
    ...
    it("routes a product block", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createBounded(CalculusOperationKind.PRODUCT, "n", "n", 1, 5),
            AngleMode.RAD,
        );
    ...
    it("surfaces service errors inline", () => {
        const resolution = createRouter().routeBlock(
            CalculusBlockDescriptor.createUnary(CalculusOperationKind.INTEGRATE, "sec(x)", "x"),
            AngleMode.RAD,
        );
