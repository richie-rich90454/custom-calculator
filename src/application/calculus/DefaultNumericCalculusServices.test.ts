...
describe("DefaultNumericDifferentiationService", () => {
  ...
  it("approximates the derivative of sine at zero", () => {
    const result = service.estimateDerivative(
      "sin(x)",
      "x",
      0,
      AngleMode.RAD
...
describe("DefaultNumericIntegrationService", () => {
  ...
  it("integrates sine over half a period", () => {
    const result = service.integrate(
      "sin(x)",
      "x",
      0,
      Math.PI,
  ...
  it("reports a clear error for a singular endpoint", () => {
    expect(() =>
      service.integrate("1/x^2", "x", 0, 1, AngleMode.RAD)
...
describe("DefaultFiniteSummationService", () => {
  ...
  it("rejects non integer bounds", () => {
    expect(() => service.sum("n", "n", 1.5, 10, AngleMode.RAD)).toThrow(
      "integers"
    );
  });

  it("rejects inverted bounds", () => {
    expect(() => service.sum("n", "n", 5, 1, AngleMode.RAD)).toThrow(
      "lower bound"
    );
  });

  it("rejects excessively large ranges", () => {
    expect(() => service.sum("n", "n", 1, 200000, AngleMode.RAD)).toThrow(
      "too large"
    );
  });
});
...
describe("DefaultFiniteProductService", () => {
  ...
  it("rejects non integer bounds", () => {
    expect(() => service.product("n", "n", 1.5, 5, AngleMode.RAD)).toThrow(
      "integers"
    );
  });

  it("rejects inverted bounds", () => {
    expect(() => service.product("n", "n", 5, 1, AngleMode.RAD)).toThrow(
      "lower bound"
    );
  });

  it("rejects excessively large ranges", () => {
    expect(() => service.product("n", "n", 1, 200000, AngleMode.RAD)).toThrow(
      "too large"
    );
  });
});
