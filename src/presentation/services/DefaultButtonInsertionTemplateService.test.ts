...
describe("DefaultButtonInsertionTemplateService", () => {
    ...
    it("resolves a template from a button definition by behavior", () => {
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_CHARACTER, "7"),
            ).text,
        ).toBe("7");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_OPERATOR, "+"),
            ).text,
        ).toBe("+");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_FUNCTION, "cos"),
            ).text,
        ).toBe("cos(");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_WRAPPED_SELECTION, "tan"),
            ).text,
        ).toBe("tan(");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_CONSTANT, "pi"),
            ).text,
        ).toBe("pi");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_VARIABLE, "ans"),
            ).text,
        ).toBe("ans");
        expect(
            service.resolveTemplate(
                createButton(CalculatorButtonInsertionBehavior.INSERT_TEMPLATE, "sum"),
            ).text,
        ).toBe("sum(, n, 1, 10)");
    });
    ...
    it("falls back to a character template for an unknown behavior", () => {
        const template = service.resolveTemplate(
            createButton("UNKNOWN" as CalculatorButtonInsertionBehavior, "?"),
        );
