...
function buildKey(
  overrides: Partial<CalculatorKeyDefinition> = {}
...
describe("CalculatorKeyCommandDispatcherService", () => {
  ...
  it("dispatches a clear press", () => {
    ...
    dispatcher.dispatchKeyPressed(
      buildKey({ kind: CalculatorKeyKind.CLEAR }),
      actions
    );
  ...
  it("dispatches an evaluate press", () => {
    ...
    dispatcher.dispatchKeyPressed(
      buildKey({ kind: CalculatorKeyKind.EVALUATE }),
      actions
    );
  ...
  it("dispatches a memory recall press", () => {
    ...
    dispatcher.dispatchKeyPressed(
      buildKey({ kind: CalculatorKeyKind.MEMORY_RECALL }),
      actions
    );
