...
describe("ConstantsPanelViewModel", () => {
    ...
    it("reports whether a search query is present", () => {
        expect(
            new ConstantsPanelViewModel(constants, " ", ConstantsPanelViewModel.ALL_CATEGORIES_ID)
                .hasSearchQuery,
        ).toBe(false);
        expect(
            new ConstantsPanelViewModel(constants, "pi", ConstantsPanelViewModel.ALL_CATEGORIES_ID)
                .hasSearchQuery,
        ).toBe(true);
    });
    ...
    it("offers an all categories option plus one per category", () => {
        ...
        expect(viewModel.categoryOptions[0]?.id).toBe(ConstantsPanelViewModel.ALL_CATEGORIES_ID);
    });
});
