...
export interface KeypadGridNavigationController {
    ...
    readonly registerItemRef: (itemId: string, element: HTMLButtonElement | null) => void;
...
export function useKeypadGridNavigation(
    ...
    const [focusedItemId, setFocusedItemId] = useState<string | null>(itemIds[0] ?? null);
    const itemRefs = useRef<Record<string, HTMLButtonElement | null>>({});

    const registerItemRef = (itemId: string, element: HTMLButtonElement | null): void => {
    ...
    const handleGridKeyDown = (event: KeyboardEvent<HTMLElement>): void => {
        const currentIndex = focusedItemId === null ? -1 : itemIds.indexOf(focusedItemId);
