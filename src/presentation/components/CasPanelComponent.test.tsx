...
describe("CasPanelComponent", () => {
    it("warns when CAS is disabled", () => {
        ...
        renderWithCalculatorContext(harness, <CasPanelComponent />);

        expect(screen.getByText(/CAS mode is disabled/)).toBeInTheDocument();
    });
    ...
    it("differentiates the current expression with the chosen variable", async () => {
        ...
        await user.click(screen.getByRole("button", { name: "Differentiate" }));
