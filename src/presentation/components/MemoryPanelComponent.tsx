    ...
    <div className={cssClass(styles.panel)}>
      <div className={cssClass(styles.valueBox)} aria-live="polite">
        {viewModel.hasMemory ? (
          ...
          <span className={cssClass(styles.emptyValue)}>Memory is empty</span>
        )}
      </div>
      ...
      <p className={cssClass(styles.hintText)}>
        M+ adds the current result to memory. M− subtracts it. MR inserts the
        memory value into the expression.
      </p>
    </div>
  );
}
