...
describe("HistoryPanelComponent", () => {
    async function recordEntry(harness: {
        store: {
            getState: () => { onDigitPressed: (d: string) => void; onEvaluatePressed: () => void };
        };
    }): Promise<void> {
    ...
    it("shows an empty state when no history exists", () => {
        ...
        renderWithCalculatorContext(harness, <HistoryPanelComponent />);
    ...
    it("inserts a history expression into the editor", async () => {
        ...
        await userEvent.setup().click(screen.getByRole("button", { name: "Insert expression 6" }));
    ...
    it("inserts a history result into the editor", async () => {
        ...
        await userEvent.setup().click(screen.getByRole("button", { name: "Insert result 6" }));
    ...
    it("deletes a history entry", async () => {
        ...
        await userEvent
            .setup()
            .click(screen.getByRole("button", { name: "Delete history entry 6" }));
