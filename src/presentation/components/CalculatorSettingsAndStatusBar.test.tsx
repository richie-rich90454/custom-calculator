...
describe("CalculatorStatusBarComponent", () => {
    ...
    it("shows the memory indicator when memory holds a value", () => {
        ...
        expect(screen.getByLabelText("Memory contains a value")).toHaveTextContent("M");
    });
    ...
    it("shows a status message when one is present", () => {
        ...
        expect(screen.getByRole("status")).toHaveTextContent("BigInt mode is unavailable.");
    });
});
...
describe("SettingsPanelComponent", () => {
    it("toggles the complex numbers setting", async () => {
        ...
        renderWithCalculatorContext(harness, <SettingsPanelComponent />);

        await user.click(screen.getByRole("switch", { name: "Enable complex numbers" }));
    ...
    it("changes the angle mode through the select", async () => {
        ...
        await user.click(screen.getByRole("button", { name: /angle mode/i }));
        await user.click(await screen.findByRole("option", { name: "Radians (RAD)" }));
    ...
    it("changes the numeric mode through the select", async () => {
        ...
        await user.click(screen.getByRole("button", { name: /numeric mode/i }));
        await user.click(await screen.findByRole("option", { name: "EXACT_DECIMAL" }));
