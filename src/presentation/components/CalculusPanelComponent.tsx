...
export function CalculusPanelComponent() {
    ...
    const operations = compositionRoot.calculusOperationCatalogService.getOperations();

    return (
        <PanelComponent title="Calculus">
            <p className={cssClass(styles.hintText)}>
                Calculus operations act on typed blocks such as derivative(x^2, x) or integral(x^2,
                x, 0, 1). Select a block below to insert its syntax.
            </p>
