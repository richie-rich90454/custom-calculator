...
export function CalculatorExpressionEditorComponent(
    ...
    const { actions, expressionText, selectionStart, selectionEnd, errorText } = props;
    ...
    const caretState = caretRenderingService.getCaretRenderingState(hasFocus, caretOffsetX);

    return (
        <TextField className={cssClass(styles.textField)}>
            <Label className={cssClass(styles.visuallyHidden)}>Calculator expression input</Label>
            <div className={cssClass(styles.editorRow)}>
                <Input
                    ...
                    autoFocus
                    {...(errorText !== null ? { "aria-describedby": "error-text" } : {})}
