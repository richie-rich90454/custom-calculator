...
describe("VariablesPanelComponent", () => {
    it("shows an empty state when no variables are saved", () => {
        ...
        renderWithCalculatorContext(harness, <VariablesPanelComponent />);
    ...
    it("inserts a saved variable into the expression", async () => {
        ...
        await user.click(screen.getByRole("button", { name: "Insert variable x" }));
    ...
    it("deletes a saved variable", async () => {
        ...
        await user.click(screen.getByRole("button", { name: "Delete variable x" }));
