...
export function ConstantsPanelListItemComponent(props: ConstantsPanelListItemComponentProperties) {
    const { constant, onInsert } = props;
    const valuePreview = `${constant.value}${constant.unit !== null ? ` ${constant.unit}` : ""}`;

    return (
        <li className={cssClass(styles.constantEntry)}>
            <div className={cssClass(styles.constantText)}>
                <span className={cssClass(styles.constantSymbol)}>{constant.symbol}</span>
