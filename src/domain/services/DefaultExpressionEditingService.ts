import type { ExpressionEditingService, ExpressionTextEdit } from "./ExpressionEditingService";
import type { ScientificFunctionCatalogService } from "./ScientificFunctionCatalogService";

export abstract class AbstractExpressionEditingService implements ExpressionEditingService {
...
export class DefaultExpressionEditingService extends AbstractExpressionEditingService {
    ...
    private static readonly LEADING_IDENTIFIER_TOKEN_PATTERN = /^[a-zA-Z][a-zA-Z0-9]*/;
    ...
    public constructor(private readonly functionCatalogService: ScientificFunctionCatalogService) {
        super();
    }
    ...
    public insertText(
        ...
        const normalizedSelectionStart = this.clampSelectionStart(selectionStart, currentText);
        const normalizedSelectionEnd = this.clampSelectionEnd(selectionEnd, currentText);
    ...
    public deleteBackward(
        ...
        const trailingFunctionGroupStart = this.findTrailingFunctionGroupStart(prefix);
        ...
        if (trailingFunctionGroupStart >= 0) {
            ...
            const removalEnd =
                closingParenthesisIndex >= 0 ? closingParenthesisIndex + 1 : cursorPosition;

            return this.removeRangeEdit(currentText, trailingFunctionGroupStart, removalEnd);
        }

        if (prefix.endsWith("(") && suffix.startsWith(")")) {
            return this.removeRangeEdit(currentText, cursorPosition - 1, cursorPosition + 1);
        }

        const trailingIdentifierStart = this.findTrailingIdentifierStart(prefix);

        if (trailingIdentifierStart >= 0) {
            return this.removeRangeEdit(currentText, trailingIdentifierStart, cursorPosition);
        }
    ...
    public deleteWordBackward(
        ...
        const prefix = currentText.slice(0, cursorPosition);
        const wordTokenMatch = prefix.match(DefaultExpressionEditingService.WORD_TOKEN_PATTERN);

        if (wordTokenMatch !== null && wordTokenMatch.index !== undefined) {
            return this.removeRangeEdit(currentText, wordTokenMatch.index, cursorPosition);
        }
    ...
    private findTrailingFunctionGroupStart(prefix: string): number {
        ...
        const nameTokenMatch = possibleNameText.match(/[a-zA-Z][a-zA-Z0-9]*$/);
    ...
    private findMatchingClosingParenthesis(text: string, openIndex: number): number {
        let depth = 0;
    ...
    private findTrailingIdentifierStart(prefix: string): number {
        const match = prefix.match(DefaultExpressionEditingService.IDENTIFIER_TOKEN_PATTERN);
    ...
    private deleteRange(
        ...
        return this.removeRangeEdit(currentText, selectionStart, selectionEnd);
    }
    ...
    private removeRangeEdit(
        ...
        const nextText = currentText.slice(0, startIndex) + currentText.slice(endIndex);
    ...
    private noOpEdit(currentText: string, cursorPosition: number): ExpressionTextEdit {
        return {
            text: currentText,
