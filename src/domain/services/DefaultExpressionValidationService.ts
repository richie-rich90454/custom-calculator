...
export class DefaultExpressionValidationService implements ExpressionValidationService {
    ...
    public validateExpression(sessionState: CalculatorSessionState): CalculationError | null {
    ...
    private validateOperatorPlacement(expressionText: string): CalculationError | null {
        if (DefaultExpressionValidationService.TRAILING_OPERATOR_PATTERN.test(expressionText)) {
            return new CalculationError(
    ...
    private validateIdentifiers(sessionState: CalculatorSessionState): CalculationError | null {
        ...
        for (const match of matches) {
            ...
            const nextCharacter = expressionText.charAt(match.index + match[0].length);
    ...
    private isUserVariable(identifier: string, sessionState: CalculatorSessionState): boolean {
        return sessionState.variables.some((variable) => variable.name === identifier);
    }
