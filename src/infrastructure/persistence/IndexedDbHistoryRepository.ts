...
export class IndexedDbHistoryRepository implements HistoryRepository {
    ...
    public async loadHistoryEntries(): Promise<readonly HistoryEntry[]> {
        const records = await this.database.history.orderBy("createdAt").reverse().toArray();

        return records.map((record) => this.database.fromHistoryEntryRecord(record));
    }

    public async saveHistoryEntry(entry: HistoryEntry): Promise<void> {
        await this.database.history.put(this.database.toHistoryEntryRecord(entry));
    }
