import type { MathJsInstance, MathType, Unit, Complex, BigNumber } from "mathjs";
...
export class MathJsConstantScopeBuilder implements MathJsEvaluationScopeBuilder {
    public constructor(
    ...
    private populateConstants(scope: Record<string, unknown>, math: MathJsInstance): void {
    ...
    private createConfiguredValue(
        ...
        if (numberType === "Fraction") {
            try {
                return this.fractionValueFactory.createFractionFromValueText(math, valueText);
            } catch {
                return null;
            }
        }
    ...
    private evaluateValueTextAsNumber(math: MathJsInstance, valueText: string): number {
    ...
    private populateTrigonometricWrappers(
        ...
        const toRadians = (value: MathType): MathType => math.multiply(value, radiansPerUnit);
        const fromRadians = (value: MathType): MathType => math.divide(value, radiansPerUnit);

        type TrigArgument = number | Unit | Complex | BigNumber;
        type InverseTrigArgument = number | Complex | BigNumber;

        const sinValue = (value: unknown): MathType => math.sin(value as never);
        const cosValue = (value: unknown): MathType => math.cos(value as never);
        const tanValue = (value: unknown): MathType => math.tan(value as never);
        const asinValue = (value: unknown): MathType => math.asin(value as never);
        const acosValue = (value: unknown): MathType => math.acos(value as never);
        const atanValue = (value: unknown): MathType => math.atan(value as never);
        ...
        scope["asin"] = (value: InverseTrigArgument): MathType => fromRadians(asinValue(value));
        scope["acos"] = (value: InverseTrigArgument): MathType => fromRadians(acosValue(value));
        scope["atan"] = (value: InverseTrigArgument): MathType => fromRadians(atanValue(value));
    }
