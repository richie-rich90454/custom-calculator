...
export class MathJsExpressionEvaluationGateway implements ExpressionEvaluationGateway {
    ...
    public evaluateExpression(sessionState: CalculatorSessionState): EvaluationResult {
    ...
    private validateExpressionNode(node: unknown, scope: Record<string, unknown>): void {
    ...
    private validateResultType(result: unknown, numericMode: NumericMode): void {
    ...
    private rejectNonFiniteResults(
        ...
        if (formattedResult.includes("Infinity") || formattedResult.includes("NaN")) {
    ...
    private createDomainErrorForResult(expressionText: string): CalculationError {
        if (MathJsExpressionEvaluationGateway.LOGARITHM_FUNCTION_PATTERN.test(expressionText)) {
    ...
    private createInfinityErrorForResult(expressionText: string): CalculationError {
        if (MathJsExpressionEvaluationGateway.LOGARITHM_FUNCTION_PATTERN.test(expressionText)) {
            return new CalculationError(
                CalculationErrorCode.DOMAIN_ERROR,
                "Domain error: the logarithm is undefined for zero or negative values.",
        ...
        if (MathJsExpressionEvaluationGateway.DIVISION_OPERATOR_PATTERN.test(expressionText)) {
            return new CalculationError(
    ...
    private formatResult(result: unknown, math: MathJsInstance, numericMode: NumericMode): string {
