...
describe("DefaultSymbolicIntegrationService", () => {
  ...
  it("integrates sine", () => {
    expect(integrationService.integrateSymbolically("sin(x)", "x")).toBe(
      "-cos(x)"
    );
  });

  it("integrates cosine", () => {
    expect(integrationService.integrateSymbolically("cos(x)", "x")).toBe(
      "sin(x)"
    );
  });

  it("integrates the exponential function", () => {
    expect(integrationService.integrateSymbolically("exp(x)", "x")).toBe(
      "exp(x)"
    );
  });

  it("integrates the reciprocal function", () => {
    expect(integrationService.integrateSymbolically("1/x", "x")).toBe(
      "log(x)"
    );
  });

  it("integrates a sum term by term", () => {
    expect(integrationService.integrateSymbolically("x^2 + 2*x", "x")).toBe(
      "x^3/3+x^2"
    );
  });
  ...
  it("integrates a difference term by term", () => {
    expect(integrationService.integrateSymbolically("x - 1", "x")).toBe(
      "x^2/2-x"
    );
  });

  it("integrates a unary minus", () => {
    expect(integrationService.integrateSymbolically("-x", "x")).toBe(
      "x^2*-1/2"
    );
  });

  it("integrates a constant quotient", () => {
    expect(integrationService.integrateSymbolically("2/3", "x")).toBe(
      "x*2/3"
    );
  });

  it("integrates a reciprocal power", () => {
    expect(integrationService.integrateSymbolically("1/x^2", "x")).toBe(
      "x^3/3"
    );
  });
  ...
  it("rejects an inverse power as unsupported", () => {
    expect(() =>
      integrationService.integrateSymbolically("x^-1", "x")
    ).toThrow("not supported");
  });

  it("integrates a natural logarithm", () => {
    expect(integrationService.integrateSymbolically("log(x)", "x")).toBe(
      "x*(log(x)-1)"
    );
  });

  it("integrates the ln alias", () => {
    expect(integrationService.integrateSymbolically("ln(x)", "x")).toBe(
      "x*(log(x)-1)"
    );
  });

  it("integrates a square root", () => {
    expect(integrationService.integrateSymbolically("sqrt(x)", "x")).toBe(
      "sqrt(x)^(3/2)*2/3"
    );
  });
  ...
  it("rejects a product of two variable factors", () => {
    expect(() =>
      integrationService.integrateSymbolically("x*x", "x")
    ).toThrow("not supported");
  });
  ...
  it("rejects a quotient with a variable numerator", () => {
    expect(() =>
      integrationService.integrateSymbolically("x/2", "x")
    ).toThrow("not supported");
  });

  it("rejects a quotient whose denominator is not a power of the variable", () => {
    expect(() =>
      integrationService.integrateSymbolically("1/(x+1)", "x")
    ).toThrow("not supported");
  });
  ...
  it("rejects an unsupported operator", () => {
    expect(() =>
      integrationService.integrateSymbolically("x mod 2", "x")
    ).toThrow("not supported");
  });

  it("rejects a non operator node type", () => {
    expect(() =>
      integrationService.integrateSymbolically("[1]", "x")
    ).toThrow("not supported");
  });

  it("rejects a power with a non symbol base", () => {
    expect(() =>
      integrationService.integrateSymbolically("2^x", "x")
    ).toThrow("not supported");
  });

  it("rejects a function with multiple arguments", () => {
    expect(() =>
      integrationService.integrateSymbolically("sin(x, 2)", "x")
    ).toThrow("not supported");
  });
