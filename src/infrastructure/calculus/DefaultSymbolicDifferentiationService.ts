...
export class DefaultSymbolicDifferentiationService implements SymbolicDifferentiationService {
    private static readonly ANGLE_AWARE_TRIG_FUNCTIONS = new Set(["sin", "cos", "tan"]);
    ...
    public differentiateSymbolically(
        ...
        const conversionFactor = this.angleModePolicyService.getSymbolicTrigConversionFactor(
            angleMode,
            policy,
        );
    ...
    private applyAngleConversion(node: unknown, factor: number): unknown {
        ...
        return mathJsNode.transform((child) => {
            ...
            const scaledArgument = new math.OperatorNode("*", "multiply", [
                argument as never,
                new math.ConstantNode(factor),
            ]);
