    ...
    public expand(
        ...
        try {
            ...
            for (let index = 0; index <= order; index += 1) {
                ...
                sumNode = new math.OperatorNode("+", "add", [sumNode, math.parse(termText)]);

                if (index < order) {
                    currentDerivativeText = this.differentiationService.differentiateSymbolically(
                        currentDerivativeText,
                        variableName,
                        angleMode,
                        policy,
            ...
            return this.resultFormattingService.normalizeResultText(math.format(simplifiedNode));
        } finally {
