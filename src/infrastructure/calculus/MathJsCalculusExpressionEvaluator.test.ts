...
describe("MathJsCalculusExpressionEvaluator", () => {
    ...
    it("respects the active angle mode for sine", () => {
        ...
        expect(evaluator.evaluate("sin(x)", "x", Math.PI / 2, AngleMode.RAD)).toBeCloseTo(1);
    });
    ...
    it("throws a clear error for non finite results", () => {
        expect(() => evaluator.evaluate("1/x", "x", 0, AngleMode.RAD)).toThrow("non finite");
    });

    it("throws a clear error for invalid expressions", () => {
        expect(() => evaluator.evaluate("x +", "x", 1, AngleMode.RAD)).toThrow("evaluation failed");
    });
    ...
    it("stringifies a non error thrown value", () => {
        ...
        expect(() => throwingEvaluator.evaluate("x", "x", 1, AngleMode.RAD)).toThrow("raw failure");
    });
});
